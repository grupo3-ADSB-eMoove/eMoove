...
function selectUltimosQuatroDiasPorSensor(idEstabelecimento, idSensor) {
    var instrucao = `
    SELECT DATE_FORMAT(c.dtHora, '%d/%c') AS entrada FROM estabelecimento e 
    JOIN localInstalado l ON e.idEstabelecimento = l.fkEstabelecimento
    JOIN sensor s ON l.idLocal = s.fkLocalInstalado
    JOIN capturaDados c ON s.idSensor = c.fkSensor
      WHERE e.idEstabelecimento = ${idEstabelecimento} AND (SELECT TIMESTAMPDIFF(DAY, c.dtHora, now())) < 4 AND s.idSensor = ${idSensor};
    `
  ...
  var instrucao = `
    SELECT COUNT(c.valor) as qtdPessoas FROM capturaDados c 
      ...
      JOIN localInstalado l ON l.idLocal = s.fkLocalInstalado 
        WHERE l.fkEstabelecimento = ${id} AND TIMESTAMPDIFF(SECOND, c.dtHora, now()) < 90`
...
function selectEntradasPorHorarioPorSensor(idEstabelecimento, horario1, horario2, idSensor) {
  ...
  var instrucao = `
    SELECT COUNT(c.valor) AS qtdEntradas FROM estabelecimento e 
        ...
        JOIN sensor s ON l.idLocal = s.fkLocalInstalado
        JOIN capturaDados c ON s.idSensor = c.fkSensor
          WHERE e.idEstabelecimento = ${idEstabelecimento} AND (SELECT TIMESTAMPDIFF(DAY, c.dtHora, now())) = 0 AND c.dtHora >= (select concat(curdate(), ' ${horario1}')) AND c.dtHora < (select concat(curdate(), ' ${horario2}')) AND s.idSensor = ${idSensor};
  `
  return database.executar(instrucao)
}
module.exports = {
